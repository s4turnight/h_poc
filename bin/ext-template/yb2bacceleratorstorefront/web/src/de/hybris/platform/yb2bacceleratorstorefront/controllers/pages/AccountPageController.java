...
public class AccountPageController extends AbstractSearchPageController
{
	...
	public String orders(@RequestParam(value = "page", defaultValue = "0") final int page,
			...
			@RequestParam(value = "unpaidFilter", defaultValue = "false", required = false) final String unpaidFilter,//add by dandelion 20140506
			...
			throws CMSItemNotFoundException
	{
		...
		final SearchPageData<OrderHistoryData> searchPageData = orderFacade.getPagedOrderHistoryForStatuses(pageableData,
				orderStatuses);
		//add by dandelion 20140506 ->
		System.out.println("method:orders.................................................");
		if ((unpaidFilter != null) && (!unpaidFilter.equals("")))
		{
			final List<OrderHistoryData> orderHistoryList = searchPageData.getResults();
			for (final OrderHistoryData ohl : orderHistoryList)
			{
				final OrderData orderDetails = orderFacade.getOrderDetailsForCode(ohl.getCode());
				System.out.println(orderDetails.getPaymentType().getCode());
				if (!orderDetails.getPaymentType().getCode().equalsIgnoreCase("PAID"))
				{
					searchPageData.getResults().remove(ohl);
				}
			}
		}
		//add by dandelion 20140506 <-
